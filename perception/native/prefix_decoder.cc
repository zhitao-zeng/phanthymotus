...
OfflineTransducerModifiedBeamSearchDecoder::Decode(
  ...
  PrefixLm *prefix_lm = lm_ && lm_scale_ > 0 ? lm_->GetPrefixLm() : nullptr;
  PrefixLmReset reset(prefix_lm);
  ...
  for (auto n : packed_encoder_out.batch_sizes) {
    ...
    // Keep ASR and prefix LM scores separate. Only this ranking array is fused.
    // Blank/unk keep the existing prefix score without advancing the LM.
    const bool use_early = prefix_lm != nullptr;
    std::vector<float> fused;
    std::vector<const PrefixState *> prefix_states;
    float *ranking = p_logprob;
    if (use_early) {
      fused.assign(p_logprob, p_logprob + num_hyps * vocab_size);
      prefix_states.reserve(num_hyps);
      for (int32_t h = 0; h < num_hyps; ++h) {
        const auto &state = prefix_lm->Get(prev[h].ys, context_size);
        prefix_states.push_back(&state);
        for (int32_t token = 0; token < vocab_size; ++token) {
          double lp = state.prefix_logprob;
          if (token != 0 && token != unk_id_) lp += state.next.at(token);
          fused[h * vocab_size + token] += static_cast<float>(lm_scale_ * lp);
        }
      }
      ranking = fused.data();
    }
    ...
    for (int32_t i = 0; i != n; ++i) {
      ...
      auto topk =
          TopkIndex(ranking, vocab_size * (end - start), max_active_paths_);
      ...
      for (auto k : topk) {
        ...
        if (use_early) {
          const auto &state = *prefix_states[hyp_index];
          double lp = state.prefix_logprob;
          if (new_token != 0 && new_token != unk_id_) lp += state.next.at(new_token);
          new_hyp.lm_log_prob = lm_scale_ * lp;
        }
        // Hypotheses::Add merges acoustic log probabilities only; same-prefix
        // paths have the same LM score, so it is not added twice.
        hyps.Add(std::move(new_hyp));
      }  // for (auto k : topk)
      p_logprob += (end - start) * vocab_size;
      ranking += (end - start) * vocab_size;
  ...
  if (prefix_lm) {
    for (auto &hyps : cur) {
      for (auto &item : hyps) {
        const auto &state = prefix_lm->Get(item.second.ys, context_size);
        // Same full-prefix probability plus one EOS in early and late modes.
        item.second.lm_log_prob =
            lm_scale_ * (state.prefix_logprob + state.next.at(1));
      }
    }
  } else if (lm_) {
