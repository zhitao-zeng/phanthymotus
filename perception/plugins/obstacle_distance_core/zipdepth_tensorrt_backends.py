...
branch therefore converts a calibrated ROI statistic directly into the two
distances consumed by the benchmark protocol.  Vehicle images continue through
the metric YOLO depth and segmentation path.
"""
...
class ZipDepthYoloTensorRTDepthBackend:
    ...
    def __init__(
        ...
        *,
        ...
        # Engines are loaded lazily on first use for the matching scene so a
        # judge run that only sees one scene (indoor or vehicle) never pays for
        # the other scene's TensorRT engine + CUDA context residency.
        ...
        self._score_threshold = _finite_number(
            indoor_config.get("score_threshold", -0.09754127264022827),
